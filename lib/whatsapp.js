...
class Message_was extends AsyncConstructor {
    constructor(client, message) {
        super(async () => {
            ...
            if (check !== "Sticker-reject" && check !== "Command-reject" && check !== "Video-reject" && check !== "Url-Reject" && check !== "Video-Url-Reject" && check !== "Url-No-Detected") {
    ...
    async checkMessage() {
        if (this.message.type === "sticker") {
            tools.conlog_info_force(`[${this.id}] rejected by it's already a sticker`);
            
            return "Sticker-reject";
        } else if (this.message.isMedia || this.message.type === "document") {
