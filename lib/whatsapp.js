...
class Whatsapp extends AsyncConstructor {
        ...
        super(async () => {
            ...
            await venom
                ...
                .then(async (client) => {
                    ...
                    await client.onStreamChange(async (state) => {
                        tools.conlog_info(`Connection status: ${state}`);
                        clearTimeout(time);
    ...
    async start() {
        ...
        tools.conlog_info_force("Loading whatsapp");

        let inchat = await this.client.isInsideChat();

        if (inchat) {
            tools.conlog_info_force("WhatsApp loaded");
        } else {
            tools.conlog_error("Error loading WhatsApp");
            return;
        }
...
class Message_was extends AsyncConstructor {
    constructor(client, message) {
        super(async () => {
            ...
            if (check !== "Video-reject" && check !== "Url-Reject" && check !== "Video-Url-Reject" && check !== "Url-No-Detected") {
            ...
            switch (check) {
                ...
                case "Url-No-Detected":
                    if (!config.notifyUrlNotDetected && check === "Url-No-Detected") {
                        break;
                    }
