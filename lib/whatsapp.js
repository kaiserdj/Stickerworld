...
async function start(client) {
    ...
    client.onMessage(async (message) => {
        ...
        if (message.chatId === "status@broadcast") {
            tools.conlog_info(`[${id}] New status detected, for user: ${await realNumber(message.author)}`);
            return;
        }
        ...
        switch (check) {
            case "Image":
                await downloadFileMessage(id, message, true);
...
async function downloadFileMessage(id, message, file) {
